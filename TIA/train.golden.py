...
#	"W1":"6u",
    ...
    print(params)
    #print("Simulating ")
    DC_gain, Freq, DB3_gain, DB3_Freq = simulate(params, scs="test", verbose=False)

      
    #normalize reward for constraints
    GAIN_MIN = 758.0
    reward = 1.0*(DC_gain - GAIN_MIN) / GAIN_MIN
    BW_MIN = 10e8
    reward += 1.0*(DB3_Freq - BW_MIN) / BW_MIN
    GBW_MIN = GAIN_MIN*BW_MIN
    reward += 1.0*(DB3_Freq*DC_gain - GBW_MIN)/GBW_MIN

    print(reward, action, DC_gain, DB3_gain, DB3_Freq, DC_gain*DB3_Freq)
 
    flog.write(str(params) + "\n")
    flog.write(str(reward) + "\t" +str(action) )
    flog.write("\t"+ str(DC_gain) +"\t"+  str(DB3_gain)+"\t" +  str(DB3_Freq)+"\t" +  str(DC_gain*DB3_Freq)+"\n")
    flog.flush()
...
def run_train_episode(start_obs, obs_top, obs_bot, agent,  rpm , EPISODE,BATCH_SIZE,WARMUP_STEPS):
        ...
        for idx in range(EPISODE):
            ...
            if rpm.size() >= WARMUP_STEPS:
		#learn from pool, learn from current
                for i in range(BATCH_SIZE): 
                    agent.learn([obs], [action], [reward], [next_state], [terminal])
...
if __name__ == "__main__":
    ...
    PRETRAIN = False #True #False #True
    PRETRAIN_COUNT = 10000
    PRELOAD =False # True
    ...
    ##############################################
    #tune K1, K2, K3, R, W1
    obs = np.array([25, 0.9, 2, 100, 10])
    #obs_top = np.array([2.0, 2.0, 2.0, 2.0, 6.0, 2.0 , 10.0 ])
    #obs_bot = np.array([0.5, 0.5, 0.5, 0.5, 3.0, 0.3, 3.0  ])
    obs_top = np.array([40, 1.2, 3, 1000, 40 ])
    obs_bot = np.array([10, 0.7, 1.0, 20, 5  ])
    ...
    for e in range(EPOCH):
        ...
        #choose the best reward parameters?
        #todos?
        #last_obs = obs
